...
export WANDB_NAME=mathcanvas-answeronly-qwen3vl-8b-full-sft

llamafactory-cli train examples/train_full/qwen3vl_full_sft_mathcanvas.yaml \
  --dataset mathcanvas_answeronly \
  --output_dir saves/qwen3-vl-8b/full/sft_answeronly_mathcanvas

TRAIN_TYPE=sft_answeronly ENABLE_TOOL_CALL=0 NUM_GPUS=8 bash scripts/inference/run_inference_qwen.sh